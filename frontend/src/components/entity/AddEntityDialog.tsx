...
const TYPES = ENTITY_TYPES.filter((item) => item.enabled);
  ...
  return (
    <div className="dialog-backdrop" role="presentation" onMouseDown={onClose}>
      <section className="nx-dialog add-entity-dialog" role="dialog" aria-modal="true" aria-label="Add entity" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <label><span>Entity type</span><select value={type} onChange={(event) => setType(event.target.value)}>{TYPES.map((item) => <option key={item.type} value={item.mapsTo || item.type}>{item.label}</option>)}</select></label>
