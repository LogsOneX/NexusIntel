...
export default function GraphTopBar({ activeCase, entities, relationships, target, setTarget, detectedType, mode, setMode, loading, onLookup, onAddEntity, onOpenPalette, onToggleDock, onToggleInspector, onToggleTerminal, onExport, onFit, onLayout }: { activeCase: Investigation | null; entities: number; relationships: number; target: string; setTarget: (value: string) => void; detectedType: string; mode: Mode; setMode: (mode: Mode) => void; loading: boolean; onLookup: (event: FormEvent<HTMLFormElement>) => void | Promise<void>; onAddEntity: () => void; onOpenPalette: () => void; onToggleDock: () => void; onToggleInspector: () => void; onToggleTerminal: () => void; onExport: () => void; onFit: () => void; onLayout: (mode: "tree" | "circular" | "force") => void }) {
  const [moreOpen, setMoreOpen] = useState(false);
  return (
    <header className="graph-topbar-v2">
      ...
      <form className="topbar-center" onSubmit={onLookup}>
        ...
        <select value={mode} onChange={(event) => setMode(event.target.value as Mode)} aria-label="Recon mode"><option value="passive">Passive</option><option value="standard">Standard</option><option value="aggressive">Aggressive</option></select>
      ...
      <section className="topbar-right">
        <button type="button" onClick={onOpenPalette}><Database size={14} /><span>Palette</span></button>
        <GraphLayoutSelector onLayout={onLayout} />
        <button className="icon-only" type="button" onClick={onToggleDock} title="Toggle case dock"><PanelLeft size={15} /></button>
        <button className="icon-only" type="button" onClick={onToggleInspector} title="Toggle inspector"><PanelRight size={15} /></button>
        <button className="icon-only" type="button" onClick={onToggleTerminal} title="Toggle telemetry"><PanelBottom size={15} /></button>
