...
type ImportType = "spiderfoot_csv" | "maltego_csv" | "ghunt_json" | "holehe_json" | "maigret_json" | "generic_ioc_csv";
...
export default function ImportWizard({ token, investigationId }: { token: string; investigationId?: string | null }) {
  ...
  const submit = async () => {
    try {
      await apiJson("/api/v1/imports", { method: "POST", body: JSON.stringify({ type: kind, investigation_id: investigationId || null, preview_only: false, rows }) }, token);
