...
type ImportType = "spiderfoot_csv" | "maltego_csv" | "generic_ioc_csv" | "urlscan_json" | "shodan_json" | "censys_json" | "virustotal_json" | "ghunt_json" | "holehe_json" | "maigret_json" | "sherlock_json" | "exiftool_json" | "nmap_xml" | "amass_output" | "subfinder_output" | "etherscan_csv" | "blockchain_csv" | "browser_bookmarks_html" | "saved_web_page_html" | "screenshot_manifest";
...
export default function ImportWizard({ token, investigationId }: { token: string; investigationId?: string | null }) {
  ...
  return (
    <section className="import-wizard">
      ...
      <div className="import-controls"><select value={kind} onChange={(event) => setKind(event.target.value as ImportType)}><option value="spiderfoot_csv">SpiderFoot CSV</option><option value="maltego_csv">Maltego CSV</option><option value="generic_ioc_csv">Generic IOC CSV</option><option value="ghunt_json">GHunt JSON</option><option value="holehe_json">Holehe JSON</option><option value="maigret_json">Maigret JSON</option><option value="sherlock_json">Sherlock JSON</option><option value="exiftool_json">ExifTool JSON</option><option value="urlscan_json">URLScan JSON</option><option value="shodan_json">Shodan JSON</option><option value="censys_json">Censys JSON</option><option value="virustotal_json">VirusTotal JSON</option><option value="nmap_xml">Nmap XML</option><option value="amass_output">Amass Output</option><option value="subfinder_output">Subfinder Output</option><option value="etherscan_csv">Etherscan CSV</option><option value="blockchain_csv">Blockchain CSV</option><option value="browser_bookmarks_html">Browser Bookmarks HTML</option><option value="saved_web_page_html">Saved Web Page HTML</option><option value="screenshot_manifest">Screenshot Manifest</option></select><label className="file-drop"><Upload size={15} />Select file<input type="file" accept=".csv,.json,.txt,.xml,.html,.htm" onChange={(event) => { const file = event.target.files?.[0]; if (file) void loadFile(file); }} /></label></div>
