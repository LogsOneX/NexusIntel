...
export default function TransformCard({ transform, selectedNode, onRun, loading }: { transform: TransformDefinition; selectedNode?: ApiNode | null; onRun?: (id: string) => void; loading?: boolean }) {
  ...
  return (
    <article className={compatible && transform.enabled ? "transform-card" : "transform-card disabled"}>
      ...
      <div className="transform-meta"><span><ShieldCheck size={12} />{transform.confidence_profile || "evidence-scored"}</span><span><Timer size={12} />{transform.runtime_profile || transform.estimated_runtime || "variable"}</span></div>
      <div className="transform-meta"><span>Connector: {transform.adapter_id || "registry"}</span><span>Noise: {transform.noise_risk || "medium"}</span></div>
      <code>{transform.input_types.join(", ")} {"->"} {transform.output_types.join(", ")}</code>
      <small>{transform.evidence_behavior || "Evidence behavior: captured when source proof is available"}</small>
