...
export default function SidebarNav({ route, user, collapsed, setCollapsed, navigate, logout }: { route: string; user: string; collapsed: boolean; setCollapsed: (value: boolean) => void; navigate: (path: string) => void; logout: () => void }) {
  return (
    <aside className="command-sidebar premium-sidebar">
      <div className="command-brand premium-brand"><Shield size={18} /><div><strong>NexusIntel</strong><span>Analyst Command</span></div></div>
      <nav aria-label="Primary navigation">
        {ROUTES.map((item) => {
          const Icon = item.icon;
          return <button aria-current={route === item.path ? "page" : undefined} className={route === item.path ? "active" : ""} key={item.path} type="button" onClick={() => navigate(item.path)} title={item.label}><Icon size={17} /><span>{item.label}</span></button>;
        })}
      </nav>
