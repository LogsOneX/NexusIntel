  ...
  return (
    <section className="dashboard-page premium-page">
      ...
      <div className="cockpit-grid">
        ...
        <section className="command-card premium-card"><header><strong>Connector Status</strong><span>BYOK</span></header><div className="connector-mini-grid"><StatusChip label="GitHub optional" tone="key" /><StatusChip label="HIBP requires key" tone="key" /><StatusChip label="URLScan optional" tone="key" /><StatusChip label="Core passive enabled" tone="ok" /></div><p className="muted-copy">Connector health is shown from configured settings when available. Missing keys disable paid/official API transforms instead of producing fake data.</p></section>
        <section className="command-card premium-card"><header><strong>Recent Evidence</strong><Database size={15} /></header><EmptyState title="Evidence summary pending" message="Evidence count appears when backend health exposes vault totals." icon={Database} /></section>
      </div>
    </section>
  );
}
