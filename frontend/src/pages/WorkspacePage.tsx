...
export default function WorkspacePage({ token, navigate }: PageProps) {
  ...
  return (
    <section className="workspace-page premium-page">
      <header className="page-header premium-page-header"><div><span className="micro-label">Workspace</span><h1>Case Management</h1></div><div className="page-actions"><button className="nx-secondary" type="button" onClick={() => setShowImport((open) => !open)}><FileUp size={15} />Import</button><button className="nx-secondary" type="button" onClick={() => navigate("/graph")}><Plus size={15} />New Investigation</button><button className="nx-secondary" type="button" onClick={autoBrief} disabled={!activeId}><Bot size={15} />AI Auto-Briefing</button></div></header>
